...
def dct_2N(x, expk=None):
    ...
    y = torch.rfft(x_pad, signal_ndim=1, normalized=False,
                   onesided=True)[..., 0:N, :]
    y.mul_(1.0/N)
...
def dct_N(x, perm=None, expk=None):
    ...
    y = torch.rfft(x_reorder, signal_ndim=1, normalized=False,
                   onesided=False)[..., 0:N, :]
    y.mul_(1.0/N)
...
def idct_2N(x, expk=None):
    ...
    # the last dimension here becomes -2 because complex numbers introduce a new dimension
    y = torch.irfft(x_pad, signal_ndim=1, normalized=False,
                    onesided=False, signal_sizes=[2*N])[..., 0:N]
    y.mul_(N)
...
def idct_N(x, expk=None):
    ...
    x_reorder[..., 0] = x.mul(expk[..., 0]).sub_(
        x_reorder[..., 1].mul(expk[..., 1]))
...
def idst(x, expkp1=None):
    ...
    # the last dimension here becomes -2 because complex numbers introduce a new dimension
    y = torch.irfft(x_pad, signal_ndim=1, normalized=False,
                    onesided=False, signal_sizes=[2*N])[..., 1:N+1]
    y.mul_(N)
...
def idxt(x, cos_or_sin_flag, expk=None):
    ...
    # the last dimension here becomes -2 because complex numbers introduce a new dimension
    # Must use IFFT here
    y = torch.ifft(x_pad, signal_ndim=1,
