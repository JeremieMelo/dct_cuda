...
__global__ void computeDctForward(const TValue *curr_ptr, TValue *next_ptr, const TValue *cos, TIndex N, TIndex len, TIndex halfLen, TIndex cosOffset)
{
    TIndex halfN = (N >> 1);
...
template <typename TValue, typename TIndex>
__global__ void computeDctBackward(const TValue *curr_ptr, TValue *next_ptr, TIndex N, TIndex len, TIndex halfLen)
{
...
template <typename TValue>
void dct_ref(const TValue *vec, TValue *out, TValue* buf, const TValue *cos0, const TValue *cos1, int M, int N)
{
    ...
    while (halfLen)
    {
        computeDctForward<<<gridSize, blockSize>>>(curr, next, cos, N, len, halfLen, cosOffset);
        cosOffset += halfLen;
        len = halfLen;
        halfLen /= 2;
    ...
    while (halfLen < N)
    {
        computeDctBackward<<<gridSize, blockSize>>>(curr, next, N, len, halfLen);
        halfLen = len;
        len *= 2;
        cudaDeviceSynchronize();
        swap(curr, next);
        ...
        )
{
    ...
    cudaMalloc((void **)&d_cos0, N * sizeof); // row
    cudaMalloc((void **)&d_cos1, M * sizeof); // column

    for(int i = 0, offset = 0; i < M; i++, offset += N)
    {    
        cudaMemcpy(d_x + offset, h_x[i], N * sizeof(T), cudaMemcpyHostToDevice);
    }
    ...
    cudaDeviceSynchronize();
    dct_ref<T>(d_x, d_y, scratch, d_cos0, d_cos1, M, N);
    cudaDeviceSynchronize();
