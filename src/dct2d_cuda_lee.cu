...
template <typename TValue>
void dct_transpose_1(const TValue *vec, TValue *out, TValue *buf, const TValue *cos, int M, int N)
{
    dim3 gridSize((N / 2 + TPB - 1) / TPB, M, 1);
    dim3 blockSize(TPB, 1, 1);
...
template <typename TValue>
void dct_transpose_2(const TValue *vec, TValue *out, TValue *buf, const TValue *cos, int M, int N)
{
    dim3 gridSize((M / 2 + TPB - 1) / TPB, N, 1);
    dim3 blockSize(TPB, 1, 1);
    ...
    const int N)
{
    ...
    #if 0
    dct_transpose_1<T>(d_x, d_y, scratch, d_cos0, M, N);
    dct_transpose_2<T>(d_y, d_x, scratch, d_cos1, M, N);
    #elif 0
